...
main :: proc() {
	...
	texLogo := rl.LoadTexture("lessons/resources/raylib_logo.png")
	texBall := rl.LoadTexture("lessons/resources/ball.png")
	texPaddle := rl.LoadTexture("lessons/resources/paddle.png")
	texBrick := rl.LoadTexture("lessons/resources/brick.png")
	font := rl.LoadFont("lesson/resources/setback.png")
		...
		case .LOGO:
			{
				...
				rl.DrawTexture(
					texLogo,
					screenWidth / 2 - texLogo.width / 2,
					screenHeight / 2 - texLogo.height / 2,
					rl.WHITE,
				)
		...
		case .TITLE:
			{
				...
				rl.DrawTextEx(font, "BLOCKS", {100, 80}, 160, 10, rl.MAROON) // Draw Title
		...
		case .GAMEPLAY:
			{
				...
				LESSON02_SHAPES :: #config(LESSON02_SHAPES, false) // define a constant using the -define command line switch. e.g: -define:FOO=true
				LESSON05_TEXTURES :: #config(LESSON05_TEXTURES, true)

				when LESSON02_SHAPES {
				...
				} else when LESSON05_TEXTURES {
					// LESSON 05: Textures loading and drawing
					rl.DrawTextureEx(texPaddle, player.position, 0.0, 1.0, rl.WHITE) // Draw player

					rl.DrawTexture(
						texBall,
						i32(ball.position.x - ball.radius / 2),
						i32(ball.position.y - ball.radius / 2),
						rl.MAROON,
					) // Draw ball
					...
					for j := 0; j < BRICKS_LINES; j += 1 {
						for i := 0; i < BRICKS_PER_LINE; i += 1 {
							if bricks[j][i].active {
								...
								if (i + j) % 2 ==
								   0 {rl.DrawTextureEx(texBrick, bricks[j][i].position, 0.0, 1.0, rl.GRAY)} else {rl.DrawTextureEx(texBrick, bricks[j][i].position, 0.0, 1.0, rl.DARKGRAY)}
							}
						}
					}
				}
		...
		case .ENDING:
			{
				...
				rl.DrawTextEx(font, "GAME FINISHED", {80, 100}, 80, 6, rl.MAROON)
